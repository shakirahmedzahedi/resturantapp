export type UserRole="ORDER_TAKER"|"KITCHEN"|"ADMIN"; export type PaymentMethod="SWISH"|"CASH";
export interface Session{username:string;password:string;role:UserRole;positionId?:number}
export interface Product{id:number;code:string;nameEn:string|null;nameBn:string;price:number;displayOrder:number;active:boolean}
export type OrderStatus="NEW"|"COMPLETED"|"CANCELLED";
export interface OrderItem{productId:number;productCode:string;nameEn:string|null;nameBn:string;quantity:number;unitPrice:number;lineTotal:number}
